...
const RePainting = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        if (data && data["Painting Services"]) {
          // Filter for subCategory === "Bathroom" services
          const bathroomServices = data["Painting Services"].filter(
            (service: any) => service.subCategory === "Re Painting"
          );
          
          console.log("Re Painting Services:", bathroomServices); // Log filtered services
          
          if (bathroomServices.length === 0) {
            setError("No Painting services found");
          }
  ...
  return (
    <ServiceComponent
      ...
      title="Professional Re Painting Service"
      subtitle="Transform your space with our expert re-painting services - book now!"
